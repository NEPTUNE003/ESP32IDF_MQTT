...
int gif_decode_start(const char *filename, uint16_t x, uint16_t y, uint16_t width, uint16_t height,
                     gif89a **out_gif, FIL **out_file)
{
    uint8_t res;
    gif89a *mygif89a;
    FIL *gfile;
    ...
    *out_file = NULL;
    ...
    if (!gfile || !mygif89a || !mygif89a->lzw) {
        res = 1; goto fail;
    }
    ...
    // 检查并解析头部
    if (gif_check_head(gfile)) { res = 2; goto fail; }
    if (gif_getinfo(gfile, mygif89a)) { res = 2; goto fail; }
...
fail:
    if (gfile) { f_close(gfile); free(gfile); }
    if (mygif89a) {
        if (mygif89a->lzw) free(mygif89a->lzw);
        free(mygif89a);
    }
    return res;
}

int gif_decode_next_frame(gif89a *gif, FIL *gfile, uint16_t x, uint16_t y, uint16_t *delay)
{
    if (!g_gif_decoding) return -1;

    uint8_t res = gif_drawimage(gfile, gif, x, y);
    if (gif->gifISD.flag & 0x80) gif_recovergctbl(gif);

    if (gif->delay) *delay = gif->delay;
    else *delay = 10;
...
void gif_decode_stop(gif89a *gif, FIL *gfile)
{
    g_gif_decoding = 0;
    if (gfile) {
        f_close(gfile);
