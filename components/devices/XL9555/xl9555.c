...
static esp_err_t xl9555_read_output_byte(uint8_t *data, size_t len)
{
    uint8_t reg_addr = XL9555_OUTPUT_PORT0_REG;  // 输出寄存器起始地址 0x02
...
uint16_t xl9555_pin_write(uint16_t pin, int val)
{
    ...
    // 关键修改：读取输出寄存器（地址 0x02/0x03），而不是输入寄存器
    xl9555_read_output_byte(w_data, 2);
...
i2c_master_dev_handle_t xl9555_get_handle(void)
{
    return xl9555_handle;
}